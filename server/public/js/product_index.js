...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _touchScroll = __webpack_require__(308);
	...
	var index = function () {
		function index() {
			...
			this.loadScroll().then(_touchScroll2.default.initScroll('.main_image'));
			this.loadCate();
		}
				...
				_jquery2.default.ajax({
					...
					success: function success(res) {
						if (res && res.status) {
							...
							//加载分类
							{
								var cate_ul = new Array();
								//加载在首页显示的分类内容
								var cate_show = new Array();
								...
								try {
									for (var _iterator2 = cate[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
										...
										cate_ul.push('<a href="/product/category?cateId=' + c.id + '">\n\t\t\t\t\t\t\t\t\t<li>\n\t\t\t\t\t\t\t\t\t<dd><img src="/images/' + c.catePicture + '"></dd>\n\t\t\t\t\t\t\t\t\t<dt>' + c.cateName + '</dt>\n\t\t\t\t\t\t\t\t\t</li>\n\t\t\t\t\t\t\t\t</a>');
										//加载在首页显示的分类内容
										if (c.isShow) {
								...
								(0, _jquery2.default)(self.cate_el).html(cate_ul.join(''));
								(0, _jquery2.default)(self.cateDetail_el).html(cate_show.join(''));
							}
						}
					},
					error: function error(err) {
